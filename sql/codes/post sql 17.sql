...
SELECT 
    p.category,                   -- 카테고리명
    COUNT(DISTINCT o.order_id) AS 주문건수,  -- 주문 건수
    SUM(o.quantity) AS 총판매수량,           -- 판매된 총 수량
    SUM(o.amount) AS 총매출액                -- 판매금액 합계
FROM orders o
JOIN products p ON o.product_id = p.product_id
GROUP BY p.category
ORDER BY 총매출액 DESC;  -- 매출액 많은 순서로 정렬
-- 카테고리별 매출 비중 
...
SELECT
    PS.CATEGORY AS 카테고리,
    PS.PRODUCT_NAME AS 제품명,
    PS.제품총매출액,
    CT.카테고리총매출액,
    ROUND(PS.제품총매출액 * 100.0 / NULLIF(CT.카테고리총매출액, 0), 2) AS 카테고리매출비중
FROM PRODUCT_SALES PS
JOIN CATEGORY_TOTAL CT ON PS.CATEGORY = CT.CATEGORY
ORDER BY PS.CATEGORY, PS.제품총매출액 DESC;
